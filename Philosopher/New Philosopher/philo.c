...
/*   Updated: 2025/05/07 16:47:07 by vloddo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_input(int argc, char **argv)
{
	...
	while (i < argc)
	{
		...
		if ((value > 200 && i == 1) || ( value <= 0 && i != 5))
			ft_error_check("Error: Check valid values", 2);
		i++;
	}
}
...
int	ft_atoi(char *argv)
{
	...
	while (argv[i] == ' ' || (argv[i] >= 9 && argv[i] <= 13))
		i++;
	if (argv[i] == '-')
		ft_error_check("Error: Negative numbers not allowed", 2);
	if (argv[i] == '+')
		i++;
	result = 0;
	while (argv[i])
	{
		if (argv[i] < '0' || argv[i] > '9')
			ft_error_check("Error: Invalid number format", 2);
		result = result * 10 + (argv[i] - '0');
		if (result > INT_MAX)
			ft_error_check("Error: Number too large", 2);
		i++;
	}
	return ((int)result);
}
...
size_t	ft_get_current_time(void)
{
	...
	if ( gettimeofday(&tv, NULL) == -1)
		ft_error_check("Error: gettimeofday failed", 2);
...
int	main(int argc, char **argv)
{
	...
	if (argc == 1 || argc < 5 || argc > 6)
		ft_error_check("Error: Insert valid number of args", 2);
	ft_check_input(argc, argv);
    ft_init_philos(&program, argv, argc);
	ft_init_program(&program, argv);
	i = 0;
	while (i < program.num_philos)
	{
		pthread_create(&program.philos[i].thread, NULL, \
			ft_philo_routine, &program.philos[i]);
		i++;
	}
	return (0);
}
