...
char	**ft_fill_map(char **argv, t_game *game)
{
	...
	if (fd == -1)
		ft_error_check(game, 5, "Error directory", 6);
	...
	while (text)
	{
		...
		if (!temp)
		{
			free(text);
			close(fd);
			ft_error_check(game, 4, "Error map", 7);
		}
		free(game->map);
		game->map = temp;
		free(text);
...
char	**process_line(t_game *game, char *text, char **temp)
{
	...
	if (len > 0 && text[len - 1] == '\n')
	text[len - 1] = '\0';
temp = malloc(sizeof(char *) * game->row);
if (!temp)
return (NULL);
i = -1;
while (++i < game->row - 1)
temp[i] = game->map[i];
temp[game->row - 1] = malloc(sizeof(char) * (ft_strlen(text) + 1));
if (!temp[game->row - 1])
{
	free(temp);
	return (NULL);
